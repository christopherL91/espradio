...
/* Not present in the C6 libphy or ROM; only called if something provides it. */
__attribute__((weak)) void phy_init_flag(void);
...
/* High-level power domain control used by wifi driver.  On the C6 the PMU
 * owns the power domains; this only pins the modem clocks on (refcounted in
 * the clocks HAL).
 */
...
/* Static PHY init data blob (128 bytes for ESP32-C6).
 * Values from esp-wifi (Rust) phy_init_data_esp32c6.rs with
 * CONFIG_ESP_PHY_MAX_TX_POWER=20 pre-applied (limit(80, 0, max) folded in).
 * Bytes 2-24: TX power limits per rate group
 * Byte 111: 0x9B
 */
static const esp_phy_init_data_t phy_init_data = { .params = {
    0x01, 0x00, 0x50, 0x50, 0x50, 0x50, 0x50, 0x4c,   /*   0 -   7 */
    0x4c, 0x4c, 0x4c, 0x48, 0x28, 0x28, 0x28, 0x28,   /*   8 -  15 */
    0x4c, 0x4c, 0x4c, 0x4c, 0x48, 0x28, 0x28, 0x28,   /*  16 -  23 */
    0x28, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,   /*  24 -  31 */
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,   /*  32 -  39 */
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,   /*  40 -  47 */
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,   /*  48 -  55 */
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,   /*  56 -  63 */
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,   /*  64 -  71 */
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00,   /*  72 -  79 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /*  80 -  87 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /*  88 -  95 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /*  96 - 103 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x9b,   /* 104 - 111 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /* 112 - 119 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,   /* 120 - 127 */
} };

/* Load PHY calibration data (from NVS or static buffer) and
 * initialize the WiFi/BT PHY for ESP32-C6.
 */
void esp_phy_load_cal_and_init(void) {
    ...
    /* Keep the USB-Serial-JTAG usable while the BBPLL is in use by the PHY. */
    phy_bbpll_en_usb(true);

    /* Always run full RF calibration; there is no persistent calibration
     * storage in this port (the NVS hooks below are stubs). */
 ...
 * In this port we keep the clocks/power domain managed by the clocks HAL,
...
void esp_phy_disable(esp_phy_modem_t modem) {
    /* Bring-up: keep RF pinned on, but trace who asks for disable.
     * Rate-limited — when a PM path cycles the RF this used to fire every
     * ~500ms and the console traffic itself distorted timing. */
    static uint32_t s_disable_calls;
    uint32_t n = s_disable_calls++;
    if (n < 3u || (n & 127u) == 0u) {
        printf("espradio: esp_phy_disable(%u) IGNORED ra=%p count=%lu\n",
               (unsigned)modem, __builtin_return_address(0), (unsigned long)(n + 1u));
    }
}
